...
def stats(rows: list[dict], top_n: int = 20):
    ...
    for r in rows:
        ...
        try:
            groups[key].append(float(r["anomaly_pct"]))
    ...
    for (big, small, sym, direction), anomalies in groups.items():
        ...
        mx  = max(anomalies)
        mn  = min(anomalies)
        # 粗略估计：超过 10pct% 的信号占比（可能覆盖手续费）
        pct_10pct% = sum(1 for a in anomalies if abs(a) >= 0.1) / n * 100
        results.append({
            "big":        big,
            "small":      small,
            "symbol":     sym,
    ...
    # ── 打印 ─────────────────────────────────────────────────────────────────
    header = f"{'大所':8s} {'小所':8s} {'合约':16s} {'方向':6s} {'次数':>6s} {'均值pct%':>8s} {'最大pct%':>8s} {'>=10pct%%':>9s}"
    print("\n" + "=" * len(header))
    ...
    for r in results[:top_n]:
        print(
            ...
            f"{r['count']:>6d} {r['avg_anom']:>8.1f} {r['max_anom']:>8.1f} {r['pct_ge10pct%']:>8.1f}%"
        )

    # ── 全局汇总 ─────────────────────────────────────────────────────────────
    total = len(rows)
    all_anom = [float(r["anomaly_pct"]) for r in rows if r.get("anomaly_pct")]
    print("-" * len(header))
    print(f"总计: {total} 条信号  |  异常均值: {sum(all_anom)/len(all_anom):.1f}pct%  |  "
          f"最大: {max(all_anom):.1f}pct%")

    # ── 按标的汇总（不区分方向和交易所对） ────────────────────────────────────
    by_sym: dict[str, list[float]] = defaultdict(list)
    for r in rows:
        try:
            by_sym[r["symbol"]].append(float(r["anomaly_pct"]))
        except (ValueError, KeyError):
            pass
    ...
    print(f"\n{'─'*40}")
    print("按标的汇总（前10）")
    print(f"{'合约':16s} {'信号数':>6s} {'均值pct%':>8s} {'最大pct%':>8s}")
    print(f"{'─'*40}")
