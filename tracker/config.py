...
# ─── 策略 A：摆盘延迟信号参数 ─────────────────────────────────────────────────
A_LEADER_WINDOW_MS  = 1000         # 检测大所 N 毫秒内的价格变动
A_LEADER_MOVE_BPS   = 5.0          # 大所触发阈值：变动超过多少 bps 才算异动
A_ANOMALY_MIN_BPS   = 3.0          # 小所相对基准的异常价差最小值（bps）
A_COOLDOWN_MS       = 2000         # 同标的同方向信号冷却时间（毫秒）
